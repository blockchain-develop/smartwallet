...
async function main() {
  ...
  let raw0 = encodeInstruction(transferInstuction);

  // twice
  let raw1 = Buffer.from(raw0);

  // transaction index
  let indexBuffer = Buffer.alloc(8);
  indexBuffer.writeBigInt64LE(toBigInt(1));

  //
  let raw = Buffer.concat([raw0, raw1, indexBuffer]);
